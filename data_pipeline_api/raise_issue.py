...
def raise_issue(
    ...
    current_group = f"{issue}:{severity}"
    if issue_type in {
        "config",
        "submission_script",
        "github_repo",
        ...
        logging.info(f"Adding issue {issue} for {type} to handle")
    elif index is None:
        reads = (
        ...
        logging.info(
            f"Raising issue {issue} for {data_product}@{version} to handle"
        )
        ...
        logging.info(f"Adding issue {issue} for {index} to handle")
